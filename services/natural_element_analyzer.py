...
class NaturalElementAnalyzer:
    ...
    def __init__(self):
        self.natural_element_categories = {
            "vegetation": {
                "keywords": [
                    ...
                    "garden", "shrub", "bush", "fern", "moss", "vine", "branch", "trunk",
                    "foliage", "greenery", "flora", "botanical", "herb", "weed", "bamboo"
                ],
                "weight": 1.0
            },
            "sky": {
                "keywords": [
                    ...
                    "weather", "blue sky", "overcast", "cumulus", "cirrus"
                ],
                "weight": 0.8
            },
            "water": {
                "keywords": [
                    "water", "lake", "river", "pond", "stream", "fountain", "pool",
                    "waterfall", "creek", "brook", "canal", "reservoir", "wetland"
                ],
                "weight": 1.0
            },
            "terrain": {
                "keywords": [
                    ...
                    "sand", "gravel", "pavement", "earth", "mud", "cliff", "hill"
                ],
                "weight": 0.6
            },
            "built_environment": {
                "keywords": [
                    ...
                    "bridge", "wall", "gate", "pavilion", "gazebo", "playground",
                    "statue", "monument", "sign", "lamp", "post"
                ],
                "weight": 0.7
            }
        }
        ...
        self.seasonal_indicators = {
            ...
            "autumn": ["fall", "yellow", "orange", "red leaves", "colorful", "changing"],
    ...
    async def analyze_natural_elements(
        ...
        try:
            ...
            overall_assessment = self._generate_overall_assessment(coverage_stats, vegetation_health_score)
            recommendations = self._generate_recommendations(coverage_stats, vegetation_health_score, seasonal_analysis)
    ...
    def _categorize_labels_by_natural_elements(
        ...
        """
        categorized = {category: [] for category in self.natural_element_categories.keys()}
    ...
    def _analyze_image_colors(self, image_content: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            vegetation_green_threshold = 100  # Minimum green value for healthy vegetation
            healthy_green_ratio = max(0.0, (mean_colors[1] - vegetation_green_threshold) / 155.0)
            ...
            return {
                ...
                "color_ratios": {"green_ratio": 0.0, "red_ratio": 0.0, "blue_ratio": 0.0},
                "brightness": 0.0,
                "color_variance": 0.0,
    ...
    def _calculate_vegetation_health_score(
        ...
        try:
            ...
            vegetation_confidence = np.mean([label["confidence"] for label in vegetation_labels])
            vegetation_count = len(vegetation_labels)
            coverage_score = min(100.0, (vegetation_confidence * 50) + (vegetation_count * 10))
    ...
    def _extract_dominant_colors(self, color_analysis: Dict[str, Any]) -> List[ColorInfo]:
        """
    ...
    def _calculate_detailed_vegetation_health(
        ...
        try:
            ...
            coverage_health_score = self._calculate_coverage_health_score(vegetation_labels)
            ...
            recommendations = self._generate_health_recommendations(
                overall_score, coverage_health_score, color_health_score, label_health_score
            )
    ...
    def _calculate_coverage_health_score(self, vegetation_labels: List[Dict[str, Any]]) -> float:
    ...
    def _generate_health_recommendations(
        ...
        if coverage_score < 40:
            recommendations.append("Low vegetation coverage detected - consider increasing plant density")
        
        if color_score < 40:
            recommendations.append("Color analysis suggests vegetation may be stressed - check watering and nutrients")
    ...
    def _create_seasonal_analysis(self, labels: List[Dict[str, Any]]) -> SeasonalAnalysis:
        """
        ...
        try:
            ...
            detected_seasons = [
                season for season, score in season_scores.items() 
                if score >= 0.3
            ]
    ...
    def _extract_enhanced_dominant_colors(self, color_analysis: Dict[str, Any]) -> List[ColorInfo]:
        """
        ...
        try:
            for category_name, labels in categorized_elements.items():
                if labels:  # Only include categories with detected elements
                    ...
                    total_weighted_confidence = sum(label["weighted_confidence"] for label in labels)
                    coverage_percentage = min(100.0, total_weighted_confidence * 50)  # Scale to percentage
    ...
    def _generate_recommendations(
        ...
        if vegetation_coverage < 20:
            recommendations.append("Consider increasing vegetation coverage for better environmental balance")
        elif vegetation_coverage > 80:
            recommendations.append("Excellent vegetation coverage - maintain current green space management")
        
        # Health-based recommendations
        if vegetation_health_score:
            if vegetation_health_score < 50:
                recommendations.append("Vegetation health needs attention - consider soil and water management")
            elif vegetation_health_score > 80:
                recommendations.append("Vegetation appears very healthy - continue current maintenance practices")
        
        # Water feature recommendations
        if coverage_stats["water_coverage"] > 30:
            recommendations.append("Significant water features detected - monitor water quality and ecosystem health")
        
        # Seasonal recommendations
        if seasonal_analysis and seasonal_analysis.primary_season:
            season = seasonal_analysis.primary_season
            if season == "winter":
                recommendations.append("Winter conditions detected - consider seasonal maintenance needs")
            elif season == "spring":
                recommendations.append("Spring growth period - optimal time for planting and maintenance")
            elif season == "summer":
                recommendations.append("Summer conditions - ensure adequate watering and shade")
            elif season == "autumn":
                recommendations.append("Autumn season - prepare for seasonal changes and leaf management")
        
        # Built environment balance
        if coverage_stats["built_environment_coverage"] > 60:
            recommendations.append("High built environment coverage - consider adding more green spaces")
    ...
    def get_analysis_summary(
        ...
        try:
            ...
            if result.vegetation_health_score and result.vegetation_health_score < 60:
                summary["recommendations"].append("Consider vegetation health assessment")
