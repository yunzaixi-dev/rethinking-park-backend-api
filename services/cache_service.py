...
from app.core.exceptions import CacheError, ServiceUnavailableError
...
class CacheService:
    ...
    def __init__(self):
        ...
        if settings.REDIS_ENABLED:
            try:
                ...
                log_error(logger, e, {"operation": "redis_initialization"})
                self.enabled = False
                # Don't raise exception - gracefully degrade to no caching
        else:
    ...
    async def get_segmentation_mask(
        self, image_hash: str, object_id: str, algorithm: str = "basic"
    ) -> Optional[Dict[str, Any]]:
    ...
    async def get_extraction_result(
        self, image_hash: str, object_id: str, extraction_params: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
    ...
    async def get_natural_elements_result(
        self, image_hash: str, analysis_depth: str = "comprehensive"
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cache_key = self._generate_cache_key(
                "natural_elements", image_hash, analysis_depth=analysis_depth
            )
    ...
    async def set_batch_processing_status(
        self, batch_id: str, status: Dict[str, Any]
    ) -> bool:
    ...
    async def invalidate_cache_version(
        ...
        try:
            ...
            logger.info(
                f"Invalidated {deleted_count} cache entries for {result_type} version {version}"
            )
        ...
        **kwargs,
        ...
        try:
            ...
            success = await self.set(
                cache_key, versioned_result, config.get("ttl_hours", 24)
            )

            if success:
                logger.debug(
                    f"Cached {result_type} with version {version}: {cache_key}"
                )
    ...
    async def implement_lru_eviction(self, max_memory_mb: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            if used_memory_mb > max_memory_mb:
                    ...
                    for key in keys:
                        try:
                            ...
                            if ttl > 0:
                                ...
                                priority_score = self._calculate_eviction_priority(
                                    key, ttl, result_type
                                )
                                cache_keys_with_access.append(
                                    (key, priority_score, ttl)
                                )
                        except Exception:
                            continue
                ...
                logger.info(
                    f"LRU eviction completed: evicted {evicted_count} keys, "
    ...
    def _calculate_eviction_priority(
        self, key: str, ttl: int, result_type: str
    ) -> float:
    ...
    def _calculate_cache_efficiency_score(
        self, hit_rate: float, total_keys: int
    ) -> float:
    ...
    def _calculate_memory_efficiency(
        ...
        return {
            ...
            "memory_efficiency_score": min(
                100.0,
                (1.0 / max(1.0, redis_info.get("mem_fragmentation_ratio", 1.0))) * 100,
            ),
        }
    ...
    async def get_analysis_result(
        ...
        try:
            ...
            elif analysis_type == "natural_elements" or analysis_type in [
                "vegetation",
                "water",
                "sky",
                "terrain",
    ...
    async def warm_cache_for_common_operations(
        ...
        try:
            ...
            for image_hash in image_hashes:
                for operation in common_operations:
                    try:
                        ...
                        cache_key = self._generate_cache_key(
                            operation["operation"], image_hash, **operation["params"]
                        )
                        ...
                        if not existing_result:
                            ...
                            logger.debug(
                                f"Cache warming needed for {operation['operation']} on {image_hash}"
                            )
                        ...
                        logger.error(
                            f"Cache warming failed for {image_hash} {operation['operation']}: {e}"
                        )
            ...
            logger.info(
                f"Cache warming completed: {warming_stats['warmed_operations']} operations warmed, "
    ...
    async def cleanup_expired_cache_entries(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cleanup_stats["memory_freed_mb"] = (memory_before - memory_after) / (
                1024 * 1024
            )
            ...
            logger.info(
                f"Cache cleanup completed: removed {cleanup_stats['expired_keys_removed']} expired keys, "
