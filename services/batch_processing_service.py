...
class BatchJob:
    ...
    def update_progress(self):
        ...
        self.completed_operations = sum(
            1 for op in self.operations if op.status == BatchOperationStatus.COMPLETED
        )
        self.failed_operations = sum(
            1 for op in self.operations if op.status == BatchOperationStatus.FAILED
        )

        finished_operations = self.completed_operations + self.failed_operations
        self.progress_percentage = (
            (finished_operations / self.total_operations * 100)
...
class BatchProcessingService:
    ...
    async def create_batch_job(
        ...
        # Cache job status
        await self.cache_service.set_batch_processing_status(
            batch_id, batch_job.to_dict()
        )

        logger.info(
            f"Created batch job {batch_id} with {len(batch_operations)} operations"
        )
        return batch_id

    def _create_batch_operation(
        self, op_def: Dict[str, Any], index: int
    ) -> BatchOperation:
        ...
        for field in required_fields:
            if field not in op_def:
                raise ValueError(
                    f"Missing required field '{field}' in operation definition"
                )
    ...
    async def _process_batch_job(self, batch_id: str):
        ...
        try:
            ...
            with self._lock:
                ...
                ] += batch_job.completed_operations
                self.stats["total_operations_failed"] += batch_job.failed_operations

            logger.info(
                f"Completed batch job {batch_id}: {batch_job.completed_operations} successful, {batch_job.failed_operations} failed"
            )
    ...
    async def _process_single_operation(self, batch_id: str, operation: BatchOperation):
        ...
        try:
            ...
            logger.debug(
                f"Completed operation {operation.operation_id} in {operation.processing_time_ms}ms"
            )
            ...
            logger.error(
                f"Operation {operation.operation_id} failed with processing error: {error_info}"
            )
            ...
            if (
                error_info.get("recoverable", False)
                and operation.retry_count < operation.max_retries
            ):
                operation.retry_count += 1
                operation.status = BatchOperationStatus.PENDING
                retry_delay = min(
                    2**operation.retry_count, 10
                )  # Exponential backoff, max 10s
                logger.info(
                    f"Retrying operation {operation.operation_id} (attempt {operation.retry_count + 1}) in {retry_delay}s"
                )
            ...
            logger.error(
                f"Operation {operation.operation_id} failed with unexpected error: {error_info}"
            )
            operation.status = BatchOperationStatus.FAILED
            operation.error_message = error_info.get("message", str(e))
            operation.end_time = datetime.now()
            ...
            if operation.retry_count < min(
                operation.max_retries, 1
            ):  # Max 1 retry for unexpected errors
                operation.retry_count += 1
                operation.status = BatchOperationStatus.PENDING
                logger.info(
                    f"Retrying operation {operation.operation_id} after unexpected error (attempt {operation.retry_count + 1})"
                )
    ...
    async def _handle_detect_objects(
        self, image_hash: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_extract_object(
        self, image_hash: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        extracted_filename = (
            f"extracted_{image_hash}_{uuid.uuid4().hex[:8]}.{output_format.lower()}"
        )
        extracted_gcs_url = await gcs_service.upload_processed_image(
            result.extracted_image_bytes, extracted_filename
        )
    ...
    async def _handle_analyze_labels(
        self, image_hash: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        filtered_labels = [
            label for label in labels if label.get("score", 0) >= confidence_threshold
        ]
    ...
    async def _handle_analyze_nature(
        self, image_hash: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_annotate_image(
        self, image_hash: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        annotated_gcs_url = await gcs_service.upload_processed_image(
            annotated_image_bytes, annotated_filename
        )
    ...
    async def get_batch_results(self, batch_id: str) -> Optional[Dict[str, Any]]:
        ...
        if (
            not batch_status
            or batch_status["status"] != BatchOperationStatus.COMPLETED.value
        ):
            return None
        ...
        total_processing_time = sum(
            op["processing_time_ms"] for op in batch_status["operations"]
        )
            ...
            "summary": {
                ...
                * 100,
                ...
                "average_processing_time_ms": (
                    total_processing_time / batch_status["total_operations"]
                    if batch_status["total_operations"] > 0
                    else 0
                ),
            },
    ...
    async def _send_completion_callback(self, batch_job: BatchJob):
        ...
        try:
            ...
            callback_data = {
                ...
                "end_time": (
                    batch_job.end_time.isoformat() if batch_job.end_time else None
                ),
            }
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        logger.info(
                            f"Callback sent successfully for batch {batch_job.batch_id}"
                        )
                    else:
                        logger.warning(
                            f"Callback failed for batch {batch_job.batch_id}: {response.status}"
                        )
    ...
    def get_service_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            running_jobs = sum(
                ...
                if job.status == BatchOperationStatus.RUNNING
            )
