...
from app.core.exceptions import ImageProcessingError, ProcessingError, ValidationError
...
class ImageProcessingService:
    ...
    def extract_by_bounding_box(
        ...
        try:
            ...
            validation_result = self.validate_extraction_request(
                image_content, bounding_box, padding
            )
            if not validation_result["valid"]:
                raise ValidationError(
                    f"Invalid extraction parameters: {'; '.join(validation_result['errors'])}",
            ...
            raise ImageProcessingError(f"Failed to load or process image: {str(e)}")
    ...
    def _apply_simple_background_removal(
        ...
        try:
            ...
            log_error(logger, e, {"operation": "background_removal"})
        ...
        for obj in objects:
            try:
                ...
                log_error(
                    logger,
                    e,
                    {"operation": "batch_extract", "object_id": obj.object_id},
                )
                continue
