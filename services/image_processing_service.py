...
class ImageProcessingService:
    """Service for simple image processing and object extraction based on bounding boxes"""
    
    def __init__(self):
        self.default_padding = 10
        self.background_threshold = 30  # Color difference threshold for background removal
        self.max_extraction_size = (2000, 2000)  # Maximum size for extracted objects
        
    def extract_by_bounding_box(
        self,
        image_content: bytes,
        bounding_box: BoundingBox,
        padding: int = None,
        ...
        try:
            ...
            original_size = ImageSize(width=pil_image.width, height=pil_image.height)
            ...
            if output_format.upper() == "PNG":
                ...
                if extracted_region.mode == 'RGBA':
                    rgb_image = Image.new('RGB', extracted_region.size, (255, 255, 255))
                    rgb_image.paste(extracted_region, mask=extracted_region.split()[-1] if extracted_region.mode == 'RGBA' else None)
                    extracted_region = rgb_image
                extracted_region.save(output_buffer, format='JPEG', quality=95, optimize=True)
            elif output_format.upper() == "WEBP":
                extracted_region.save(output_buffer, format='WEBP', quality=95, optimize=True)
            else:
                # Default to PNG
                extracted_region.save(output_buffer, format='PNG', optimize=True)
            
            extracted_size = ImageSize(width=extracted_region.width, height=extracted_region.height)
            ...
            return SimpleExtractionResult(
                ...
                processing_method="bounding_box_with_padding" if padding > 0 else "bounding_box"
            )
    ...
    def _estimate_background_color(self, img_array: np.ndarray) -> Tuple[int, int, int]:
        ...
        all_corner_pixels = np.concatenate([corner.reshape(-1, img_array.shape[2]) for corner in corners])
    ...
    def _create_background_mask(
        ...
        color_diff = np.sqrt(np.sum((img_array[:, :, :3] - background_color) ** 2, axis=2))
    ...
    def _resize_if_needed(self, image: Image.Image) -> Image.Image:
        ...
        if image.width <= self.max_extraction_size[0] and image.height <= self.max_extraction_size[1]:
            return image
    ...
    def batch_extract_objects(
        ...
        for obj in objects:
            try:
                result = self.extract_by_bounding_box(
                ...
                result.extraction_id = f"extract_{obj.object_id}_{uuid.uuid4().hex[:8]}"
                results.append(result)
                
            except Exception as e:
                logger.error(f"Failed to extract object {obj.object_id}: {e}")
                continue
        
        return results
    
    def extract_with_custom_region(
        self,
        image_content: bytes,
        x: float,
        y: float,
        ...
        return self.extract_by_bounding_box(
            image_content=image_content,
    ...
    def enhance_extracted_object(
        ...
        try:
            ...
            pil_image = Image.open(io.BytesIO(extracted_result.extracted_image_bytes))
            ...
            if enhance_color:
                ...
                pil_image = enhancer.enhance(1.1)  # Increase color saturation by 10%
    ...
    def get_extraction_statistics(
        ...
        if bounding_box.x <= 1.0 and bounding_box.y <= 1.0:
            bbox_area = (bounding_box.width * original_size.width) * (bounding_box.height * original_size.height)
        else:
            bbox_area = bounding_box.width * bounding_box.height
        
        return {
            "original_dimensions": {"width": original_size.width, "height": original_size.height},
            "extracted_dimensions": {"width": extracted_size.width, "height": extracted_size.height},
            "area_ratio": extracted_area / original_area if original_area > 0 else 0,
            "size_reduction_ratio": extracted_area / original_area if original_area > 0 else 0,
            "bounding_box_area": bbox_area,
            "extraction_efficiency": bbox_area / extracted_area if extracted_area > 0 else 0
        }
    ...
    def validate_extraction_request(
        ...
        try:
            ...
            if bounding_box.x < 0 or bounding_box.y < 0:
                validation_result["errors"].append("Bounding box coordinates cannot be negative")
            
            if bounding_box.width <= 0 or bounding_box.height <= 0:
                validation_result["errors"].append("Bounding box dimensions must be positive")
            
            # Check if bounding box is within image bounds
            if bounding_box.x <= 1.0 and bounding_box.y <= 1.0:
                # Normalized coordinates
                if (bounding_box.x + bounding_box.width > 1.0 or 
                    bounding_box.y + bounding_box.height > 1.0):
                    validation_result["errors"].append("Bounding box extends beyond image boundaries")
            else:
                # Pixel coordinates
                if (bounding_box.x + bounding_box.width > image_size.width or 
                    bounding_box.y + bounding_box.height > image_size.height):
                    validation_result["errors"].append("Bounding box extends beyond image boundaries")
            ...
            if extraction_width > 2000 or extraction_height > 2000:
                validation_result["warnings"].append("Extracted region is very large and will be resized")
