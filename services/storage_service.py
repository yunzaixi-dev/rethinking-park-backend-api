...
from models.image import DuplicateCheckResponse, ImageInfo
...
class StorageService:
    ...
    async def check_duplicates(
        self, image_hash: str, perceptual_hash: Optional[str] = None
                        ...
                        similar_images.append(
                            {
                                ...
                                "similarity_score": hash_service.hamming_distance(
                                    perceptual_hash, stored_perceptual_hash
                                ),
    ...
    async def update_analysis_results(
        self, image_hash: str, analysis_results: Dict[str, Any]
    ) -> bool:
        """更新分析结果（基于哈希）"""
        try:
            # 查找对应的图像ID
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        processed_images = sum(
            1 for data in self.data.values() if data.get("processed", False)
        )
