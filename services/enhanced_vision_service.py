...
from app.core.exceptions import ProcessingError as UnifiedProcessingError
...
class EnhancedVisionService(VisionService):
    ...
    @error_handler("detect_objects_enhanced", reraise=True)
    async def detect_objects_enhanced(
        ...
        try:
            ...
            objects = await self._detect_objects_enhanced_with_retry(
                image, confidence_threshold, max_results
            )
            ...
            if include_faces:
                try:
                    face_response = (
                        await self.face_detection_service.detect_faces_enhanced(
                            image_content,
                    ...
                    logger.warning(
                        f"Face detection failed, continuing without faces: {face_error}"
                    )
            ...
            if include_labels:
                try:
                    labels = await self._detect_labels_enhanced_with_retry(
                        image, confidence_threshold
                    )
                except Exception as label_error:
                    logger.warning(
                        f"Label detection failed, continuing without labels: {label_error}"
                    )
            ...
            # Handle Vision API errors with unified error handling
            log_error(
                logger, e, {"operation": "enhanced_detection", "image_hash": image_hash}
            )
            raise VisionServiceError(f"Vision API error: {str(e)}")

        except Exception as e:
            # Handle other processing errors with unified error handling
            log_error(
                logger, e, {"operation": "enhanced_detection", "image_hash": image_hash}
            )
            raise UnifiedProcessingError(
                f"Enhanced detection failed: {str(e)}", operation="enhanced_detection"
            )
    ...
    async def _detect_objects_enhanced_with_retry(
        ...
        try:
            ...
            raise ProcessingException(
                f"Object detection processing failed: {str(e)}", "object_detection"
            )
    ...
    async def _detect_objects_enhanced(
        ...
        try:
            return await self._detect_objects_enhanced_with_retry(
                image, confidence_threshold, max_results
            )
        except Exception as e:
            logger.error(f"Enhanced object detection failed: {e}")
    ...
    async def _detect_labels_enhanced_with_retry(
        self, image: vision.Image, confidence_threshold: float = 0.5
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise ProcessingException(
                f"Label detection processing failed: {str(e)}", "label_detection"
            )
    ...
    async def _detect_labels_enhanced(
        ...
        try:
            return await self._detect_labels_enhanced_with_retry(
                image, confidence_threshold
            )
        except Exception as e:
            logger.error(f"Enhanced label detection failed: {e}")
    ...
    async def analyze_natural_elements(self, image_content: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            labels_response = await self._detect_labels_enhanced(
                image, confidence_threshold=0.3
            )
            ...
            return {"natural_elements": {}, "error": str(e), "enabled": True}

    def _categorize_natural_elements(
        ...
        if total_coverage > 0:
            for key in coverage_stats:
                coverage_stats[key] = min(
                    100.0, (coverage_stats[key] / total_coverage) * 100
                )

        return {"categories": categories, "coverage_statistics": coverage_stats}
    ...
    def _calculate_vegetation_health(
        self, natural_elements: Dict[str, Any], color_analysis: Dict[str, Any]
    ) -> Optional[float]:
        """Calculate vegetation health score based on coverage and color analysis"""
        try:
            vegetation_coverage = natural_elements.get("coverage_statistics", {}).get(
                "vegetation_coverage", 0.0
            )
    ...
    def apply_confidence_filtering(
        ...
        List[EnhancedDetectionResult], List[FaceDetectionResult], List[Dict[str, Any]]
    ]:
        """
        ...
        filtered_labels = [
            label
            for label in labels
            if label.get("confidence", 0.0) >= confidence_threshold
        ]

        return filtered_objects, filtered_faces, filtered_labels

    def calculate_detection_quality_metrics(
        self, objects: List[EnhancedDetectionResult], faces: List[FaceDetectionResult]
    ) -> Dict[str, Any]:
        """
        ...
        if objects:
            ...
            metrics["object_confidence_stats"] = {
                ...
                "high_confidence_count": len(
                    [c for c in object_confidences if c >= 0.8]
                ),
                "medium_confidence_count": len(
                    [c for c in object_confidences if 0.5 <= c < 0.8]
                ),
        ...
        if faces:
            ...
            metrics["face_confidence_stats"] = {
                ...
                "medium_confidence_count": len(
                    [c for c in face_confidences if 0.5 <= c < 0.8]
                ),
        ...
        if total_detections > 0:
            ...
            high_confidence_ratio = (
                len([c for c in all_confidences if c >= 0.8]) / total_detections
            )

            # Quality score based on average confidence and high confidence ratio
            metrics["detection_quality_score"] = (
                average_confidence * 0.6 + high_confidence_ratio * 0.4
            ) * 100
    ...
    async def detect_with_position_marking(
        ...
        try:
            ...
            filtered_objects, filtered_faces, filtered_labels = (
            ...
            quality_metrics = self.calculate_detection_quality_metrics(
                filtered_objects, filtered_faces
            )
