        ...
        with self._lock:
            ...
            if response_time_ms > 0:
                ...
                if self.request_times:
                    self.api_metrics.average_response_time_ms = sum(self.request_times) / len(self.request_times)
...
class MonitoringService:
    ...
    def _register_default_health_checks(self):
        """Register default health checks"""
        self.health_checker.register_check("database", self._check_database, timeout=5.0)
        self.health_checker.register_check("cache", self._check_cache, timeout=3.0)
        self.health_checker.register_check("vision_api", self._check_vision_api, timeout=10.0)
        self.health_checker.register_check("storage", self._check_storage, timeout=5.0)
        self.health_checker.register_check("performance", self._check_performance, timeout=2.0)
    ...
    async def _check_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result = await storage_service.health_check() if hasattr(storage_service, 'health_check') else True
            
            return {
                "status": "healthy" if test_result else "unhealthy",
                "details": {
                    "type": "storage_service",
                    "responsive": test_result
                }
            }
        except Exception as e:
            return {
    ...
    async def _check_cache(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Test write
            write_success = await self.cache_service.set(test_key, test_value, ttl_hours=1)
    ...
    async def _check_storage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status = await gcs_service.health_check() if hasattr(gcs_service, 'health_check') else True
