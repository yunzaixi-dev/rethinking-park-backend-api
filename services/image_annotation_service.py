...
class ImageAnnotationService:
        ...
        try:
            ...
            if include_object_boxes and objects:
                self._render_bounding_boxes(
                    draw, objects, img_width, img_height, styles
                )
    ...
    def _render_labels_with_connections(
        ...
        for idx, obj in enumerate(objects):
            ...
            obj_center_x = (
                obj.center_point.x * img_width
                if obj.center_point.x <= 1.0
                else obj.center_point.x
            )
            obj_center_y = (
                obj.center_point.y * img_height
                if obj.center_point.y <= 1.0
                else obj.center_point.y
            )

            # Draw line from label position to object center
            draw.line(
                [
                    (label_start_x, current_y + font_size // 2),
            ...
            if font:
                text_bbox = draw.textbbox(
                    (label_start_x, current_y), label_text, font=font
                )
                text_bg_color = (
                    *self._hex_to_rgb(styles.get("text_background_color", "#FFFFFF")),
                ...
                # Draw label text
                draw.text(
                    (label_start_x, current_y), label_text, fill=label_color, font=font
                )
            else:
        ...
        if labels:
            ...
            for label in labels[:5]:  # Limit to top 5 labels
                label_text = (
                    f"{label.get('name', 'Unknown')} ({label.get('confidence', 0):.2f})"
                )
                ...
                if font:
                    ...
                    text_bg_color = (
                        *self._hex_to_rgb(
                            styles.get("text_background_color", "#FFFFFF")
                        ),
                        styles.get("text_background_alpha", 180),
                    )
                    draw.rectangle(text_bbox, fill=text_bg_color)
                    draw.text(
                        (label_start_x, current_y),
                        label_text,
                        fill=label_color,
                        font=font,
                    )
    ...
    def create_annotation_overlay(
        ...
        try:
            ...
            overlay = Image.new(
                "RGBA", (image_size.width, image_size.height), (0, 0, 0, 0)
            )
            ...
            if objects:
                self._render_bounding_boxes(
                    draw, objects, image_size.width, image_size.height, styles
                )

            if faces:
                self._render_face_markers(
                    draw, faces, image_size.width, image_size.height, styles
                )
        ...
        try:
            ...
            if pil_image.width > 4000 or pil_image.height > 4000:
                validation_result["warnings"].append(
                    "Large image size may affect rendering performance"
                )
                ...
                for obj in objects:
                    ...
                    if not (
                        ...
                        if not (
                            bbox.x >= 0
                            and bbox.y >= 0
                            and bbox.x + bbox.width <= pil_image.width
                            and bbox.y + bbox.height <= pil_image.height
                        ):
                            validation_result["errors"].append(
                                f"Invalid bounding box for object {obj.object_id}"
                            )
            ...
            if faces:
                for face in faces:
                    ...
                    if not (0 <= center.x <= 1 and 0 <= center.y <= 1):
                        if not (
                            0 <= center.x <= pil_image.width
                            and 0 <= center.y <= pil_image.height
                        ):
                            validation_result["errors"].append(
                                f"Invalid center point for face {face.face_id}"
                            )

            # Check annotation density
            total_annotations = (len(objects) if objects else 0) + (
                len(faces) if faces else 0
            )
            if total_annotations > 100:
                validation_result["warnings"].append(
                    "High annotation density may affect readability"
                )
