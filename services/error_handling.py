...
class BatchProcessingException(Exception):
    ...
    def __init__(self, message: str, failed_items: List[Dict[str, Any]], partial_results: Optional[List[Dict[str, Any]]] = None):
        self.message = message
...
class AnnotationRenderingException(Exception):
    ...
    def __init__(self, message: str, annotation_type: str, fallback_available: bool = True):
        self.message = message
...
class LinearBackoffStrategy(RetryStrategy):
    ...
    def __init__(self, max_attempts: int = 3, base_delay: float = 1.0, increment: float = 1.0):
        super().__init__(max_attempts, base_delay)
...
def retry_vision_api(max_attempts: int = 3, base_delay: float = 1.0):
    ...
    return retry_on_exception(
        ...
        on_retry=lambda e, attempt, delay: logger.warning(f"Vision API retry {attempt}: {e}")
    )
...
def retry_processing(max_attempts: int = 2, base_delay: float = 0.5):
    ...
    return retry_on_exception(
        ...
        on_retry=lambda e, attempt, delay: logger.warning(f"Processing retry {attempt}: {e}")
    )
    ...
    def handle_error(self, error: Exception, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(error, GoogleCloudError):
            ...
            if hasattr(error, 'code'):
                ...
                if error.code in [429, 503, 500]:  # Rate limit, service unavailable, internal error
                    error_info["recoverable"] = True
                    error_info["retry_after"] = 5
                elif error.code in [400, 401, 403]:  # Bad request, unauthorized, forbidden
    ...
    def handle_error(self, error: Exception, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(error, ProcessingException):
            ...
            if error.operation == "extraction":
                error_info["fallback_suggestion"] = "Try with different bounding box coordinates"
            elif error.operation == "annotation":
                error_info["fallback_suggestion"] = "Return original image without annotations"
...
class FallbackStrategy:
    ...
    def execute_fallback(self, original_error: Exception, context: Dict[str, Any]) -> Any:
...
class ErrorRecoveryManager:
    ...
    def _classify_error(self, error: Exception) -> str:
        ...
        if isinstance(error, (GoogleCloudError, VisionAPIException, DefaultCredentialsError)):
            return "vision_api"
...
error_recovery_manager.register_fallback("batch_processing", GracefulDegradationStrategy())
...
def handle_processing_error(error: Exception, operation: str, **kwargs) -> Dict[str, Any]:
...
def handle_batch_error(error: Exception, batch_id: str, items: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def convert_to_http_exception(error_info: Dict[str, Any]) -> HTTPException:
    ...
    if error_type == "vision_api_error":
        ...
        if error_code == 429:
            ...
            return HTTPException(status_code=503, detail="Vision service temporarily unavailable")
        else:
            return HTTPException(status_code=500, detail=f"Vision API error: {message}")
    
    elif error_type == "credentials_error":
        return HTTPException(status_code=503, detail="Vision service configuration error")
        ...
        if error_info.get("partial_success", False):
            ...
            return HTTPException(status_code=500, detail=f"Batch processing failed: {message}")
