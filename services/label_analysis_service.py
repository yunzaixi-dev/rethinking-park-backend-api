...
from models.image import ColorInfo, NaturalElementsResult
...
class LabelAnalysisService:
    ...
    def analyze_by_labels(
        ...
        try:
            ...
            if image_content and analysis_depth in ["comprehensive", "detailed"]:
                color_analysis = self._analyze_image_colors_for_labels(
                    image_content, categorized_elements
                )

            # Generate natural element insights
            insights = self._generate_natural_element_insights(
                categorized_elements, coverage_stats
            )
            ...
            if include_confidence:
                analysis_result["confidence_analysis"] = self._analyze_label_confidence(
                    labels
                )
    ...
    def _estimate_coverage_from_labels(
        self, categorized_elements: Dict[str, Any]
    ) -> Dict[str, float]:
        ...
        if total_weighted_score > 0:
            for category, score in category_scores.items():
                ...
                if coverage_key in coverage_stats:
                    coverage_stats[coverage_key] = min(
                        100.0, (score / total_weighted_score) * 100
                    )

        return coverage_stats

    def _analyze_image_colors_for_labels(
        self, image_content: bytes, categorized_elements: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _calculate_color_statistics(self, img_array: np.ndarray) -> Dict[str, Any]:
        ...
        color_hist = {
            "red_distribution": np.histogram(
                img_array[:, :, 0], bins=10, range=(0, 255)
            )[0].tolist(),
            "green_distribution": np.histogram(
                img_array[:, :, 1], bins=10, range=(0, 255)
            )[0].tolist(),
            "blue_distribution": np.histogram(
                img_array[:, :, 2], bins=10, range=(0, 255)
            )[0].tolist(),
        }
    ...
    def _correlate_colors_with_categories(
        self, color_stats: Dict[str, Any], categorized_elements: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if vegetation_labels:
            correlations["vegetation_indicators"] = {
                ...
                "vegetation_health_estimate": min(
                    100, green_ratio * 150
                ),  # Scale green ratio
                "label_confidence": sum(
                    label.get("confidence", 0) for label in vegetation_labels
                )
                / len(vegetation_labels),
            }
        ...
        if sky_labels:
            correlations["sky_indicators"] = {
                ...
                "sky_clarity_estimate": min(
                    100, (blue_ratio * brightness) / 2.55
                ),  # Combine blue and brightness
                "label_confidence": sum(
                    label.get("confidence", 0) for label in sky_labels
                )
                / len(sky_labels),
            }
        ...
        if water_labels:
            correlations["water_indicators"] = {
                ...
                "label_confidence": sum(
                    label.get("confidence", 0) for label in water_labels
                )
                / len(water_labels),
            }
    ...
    def _generate_natural_element_insights(
        self, categorized_elements: Dict[str, Any], coverage_stats: Dict[str, float]
    ) -> Dict[str, Any]:
        ...
        vegetation_labels = categorized_elements.get("vegetation", [])
        if vegetation_labels:
            unique_vegetation_types = len(
                set(label.get("name", "").lower() for label in vegetation_labels)
            )
        ...
        for category_labels in categorized_elements.values():
            all_labels.extend(
                [label.get("name", "").lower() for label in category_labels]
            )
        ...
        if vegetation_coverage < 20:
            insights["recommendations"].append(
                "Consider adding more vegetation for better environmental balance"
            )
        if built_coverage > 60:
            insights["recommendations"].append(
                "High built environment coverage detected - consider green spaces"
            )
        if len(vegetation_labels) < 3:
            insights["recommendations"].append(
                "Limited vegetation diversity - consider diverse plantings"
            )
    ...
    def _analyze_label_confidence(self, labels: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        high_conf = [
            c for c in confidences if c >= self.confidence_thresholds["high_confidence"]
        ]
        medium_conf = [
            c
            for c in confidences
            if self.confidence_thresholds["medium_confidence"]
            <= c
        ...
        low_conf = [
            c
            for c in confidences
            if c < self.confidence_thresholds["medium_confidence"]
        ]
    ...
    def create_natural_elements_result(
        ...
        if color_analysis and "vegetation_indicators" in color_analysis:
            vegetation_health_score = color_analysis["vegetation_indicators"].get(
                "vegetation_health_estimate"
            )
        elif insights:
        ...
        return NaturalElementsResult(
            ...
            built_environment_coverage=coverage_stats.get(
                "built_environment_coverage", 0.0
            ),
