...
class TestVisionAPIBatcher:
    ...
    async def test_batch_request_processing(self, batcher):
        ...
        with patch(
            "services.performance_optimizer.enhanced_vision_service"
        ) as mock_service:
            mock_service.detect_objects_enhanced = AsyncMock(
                return_value={"success": True, "objects": [], "faces": [], "labels": []}
            )
...
class TestPerformanceOptimizer:
    ...
    async def test_performance_metrics_collection(self, optimizer):
        ...
        assert (
            metrics["api_calls"]["batch_efficiency_ratio"] == 5 / 8
        )  # 5 batched out of 8 total
        assert (
            metrics["cache_performance"]["hit_rate"] == 10 / 12
        )  # 10 hits out of 12 total
...
class TestIntegrationScenarios:
    ...
    async def test_concurrent_requests_optimization(self):
        ...
        try:
            ...
            with patch(
                ...
                metrics = optimizer.get_performance_metrics()
                assert (
                    metrics["api_calls"]["batched"] > 0
                    or metrics["api_calls"]["individual"] > 0
                )
    ...
    async def test_batch_processing_optimization(self):
        ...
        try:
            ...
            with patch(
                "services.performance_optimizer.storage_service"
            ) as mock_storage, patch(
                "services.performance_optimizer.gcs_service"
            ) as mock_gcs, patch(
                "services.performance_optimizer.batch_processing_service"
            ) as mock_batch:

                # Mock storage and GCS services
                mock_storage.get_image_info = AsyncMock(
                    return_value=Mock(gcs_url="test_url")
                )
                ...
                batch_id = await optimizer.optimize_batch_processing(
                    operations=operations, max_concurrent=3
                )
