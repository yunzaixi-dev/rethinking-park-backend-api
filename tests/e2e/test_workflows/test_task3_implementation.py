    ...
    objects = [
        EnhancedDetectionResult(
            ...
            bounding_box=BoundingBox(
                x=0.125, y=0.667, width=0.25, height=0.267
            ),  # Green rectangle
        ...
        EnhancedDetectionResult(
            ...
            confidence=0.75,
            bounding_box=BoundingBox(
                x=0.5, y=0.733, width=0.375, height=0.233
            ),  # Brown rectangle
...
def test_image_annotation_service():
    ...
    try:
        ...
        assert len(annotated_image) > len(
            image_content
        ), "Annotated image should be larger"
        ...
        boxes_only = image_annotation_service.render_bounding_boxes_only(
            image_content=image_content, objects=objects
        )
        ...
        faces_only = image_annotation_service.render_face_markers_only(
            image_content=image_content, faces=faces
        )
        ...
        ), f"Expected 2 objects, got {stats['total_objects']}"
        ...
        validation = image_annotation_service.validate_annotation_request(
            image_content=image_content, objects=objects, faces=faces
        )
...
def test_image_processing_service():
    ...
    try:
        ...
        batch_results = image_processing_service.batch_extract_objects(
            image_content=image_content, objects=objects, padding=5, output_format="PNG"
        )

        assert (
            len(batch_results) == 2
        ), f"Expected 2 extractions, got {len(batch_results)}"
...
def test_label_analysis_service():
    ...
    try:
        ...
        basic_analysis = label_analysis_service.analyze_by_labels(
            labels=labels, analysis_depth="basic", include_confidence=True
        )
        ...
        assert (
            natural_result.vegetation_coverage >= 0
        ), "Vegetation coverage should be non-negative"
        ...
        assert (
            "confidence_distribution" in confidence_analysis
...
def test_integration():
    ...
    try:
        ...
        label_analysis = label_analysis_service.analyze_by_labels(
            labels=labels, image_content=image_content, analysis_depth="comprehensive"
        )

        # Step 2: Extract objects
        print("  - Step 2: Extracting objects...")
        extractions = image_processing_service.batch_extract_objects(
            image_content=image_content, objects=objects, padding=10
        )

        # Step 3: Create annotated image
        print("  - Step 3: Creating annotated image...")
        annotated_image = image_annotation_service.render_annotated_image(
            image_content=image_content, objects=objects, faces=faces, labels=labels
        )
        ...
        summary = {
            "label_analysis_summary": {
                ...
                "vegetation_coverage": label_analysis["coverage_statistics"][
                    "vegetation_coverage"
                ],
        ...
        print(
            f"    • Labels analyzed: {summary['label_analysis_summary']['total_labels']}"
        )
        print(
            f"    • Vegetation coverage: {summary['label_analysis_summary']['vegetation_coverage']:.1f}%"
        )
        print(
            f"    • Objects extracted: {summary['extraction_summary']['total_extractions']}"
        )
        print(
            f"    • Annotated image size: {summary['annotation_summary']['annotated_image_size']} bytes"
        )
...
def main():
    ...
    if passed == total:
        ...
        print(
            "✓ ImageAnnotationService - Renders bounding boxes, face markers, and labels"
        )
        print("✓ ImageProcessingService - Extracts objects using bounding boxes")
        print(
            "✓ LabelAnalysisService - Analyzes natural elements from Google Vision labels"
        )
