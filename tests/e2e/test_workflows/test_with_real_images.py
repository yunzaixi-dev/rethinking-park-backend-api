...
class RealImageTester:
    ...
    def create_real_test_images(self):
        ...
        for y in range(200):
            ...
            draw.rectangle(
                [0, y, 800, y + 1], fill=(color_intensity, color_intensity + 20, 255)
            )
        ...
        for name, image_data in images.items():
            ...
            try:
                ...
                response = self.session.post(
                    f"{self.api_v1}/upload", files=files, timeout=30
                )
                ...
                if response.status_code == 200:
                    ...
                    self.uploaded_images.append(
                        {"name": name, "hash": image_hash, "data": data}
                    )
    ...
    def test_image_analysis(self):
        ...
        for img_info in self.uploaded_images:
            ...
            try:
                ...
                response = self.session.post(
                    f"{self.api_v1}/analyze", json=payload, timeout=60
                )
                ...
                if response.status_code == 200:
                    ...
                    if "labels" in data and data["labels"]:
                        ...
                        for label in data["labels"][:3]:  # 显示前3个
                            print(
                                f"     - {label.get('description', 'N/A')}: {label.get('score', 0):.2f}"
                            )
                    ...
                    if "objects" in data and data["objects"]:
                        ...
                        for obj in data["objects"][:3]:  # 显示前3个
                            print(
                                f"     - {obj.get('name', 'N/A')}: {obj.get('score', 0):.2f}"
                            )
            ...
            try:
                ...
                response = self.session.post(
                    f"{self.api_v1}/analyze-nature", json=payload, timeout=60
                )
                    ...
                    if "natural_elements" in data:
                        ...
                        for element_type, info in elements.items():
                            if isinstance(info, dict) and "confidence" in info:
                                print(
                                    f"     - {element_type}: {info['confidence']:.2f}"
                                )
        ...
        for img_info in self.uploaded_images[:2]:  # 只测试前2张
            ...
            try:
                response = self.session.get(
                    f"{self.api_v1}/image/{image_hash}", timeout=30
                )
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"   处理状态: {'已处理' if data.get('processed', False) else '未处理'}"
                    )
