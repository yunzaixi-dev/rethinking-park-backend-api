...
async def setup_test_image():
    ...
    response = client.post(
        "/api/v1/upload", files={"file": ("test_park.png", image_content, "image/png")}
    )
...
def test_label_analysis_request_validation():
    ...
    if response.status_code == 422:
        ...
        print(
            f"❌ Expected validation error for threshold, got: {response.status_code}"
        )
        return False
    ...
    if response.status_code == 422:
        ...
        print(
            f"❌ Expected validation error for max_labels, got: {response.status_code}"
        )
        return False
...
async def test_label_analysis_with_real_image():
    ...
    await cache_service.set_analysis_result(
        image_hash, "labels", {"labels": mock_labels}
    )
    ...
    if not response_data["success"]:
        print(
            f"❌ Analysis failed: {response_data.get('error_message', 'Unknown error')}"
        )
        return False
    ...
    for category in results["category_analysis"]:
        print(
            f"  🏷️ {category['category_name']}: {category['label_count']} labels, {category['coverage_estimate']:.1f}% coverage"
        )
...
async def main():
    ...
    if passed == total:
        print(
            "🎉 All integration tests passed! Label analysis endpoint is fully functional."
        )
        return True
    else:
