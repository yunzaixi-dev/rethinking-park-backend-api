...
def test_service_imports():
    ...
    try:
        ...
        assert (
            natural_element_analyzer is not None
        ), "Natural element analyzer should be available"
        print("✅ NaturalElementAnalyzer service imported successfully")

        # Test that enhanced vision service is available
        assert (
            enhanced_vision_service is not None
        ), "Enhanced vision service should be available"
        print("✅ EnhancedVisionService imported successfully")

        # Test that the analyzer has the expected methods
        assert hasattr(
            natural_element_analyzer, "analyze_natural_elements"
        ), "Should have analyze_natural_elements method"
        assert hasattr(
            natural_element_analyzer, "get_analysis_summary"
        ), "Should have get_analysis_summary method"
        print("✅ NaturalElementAnalyzer has expected methods")

        # Test that enhanced vision service has natural elements analysis
        assert hasattr(
            enhanced_vision_service, "analyze_natural_elements"
        ), "Should have analyze_natural_elements method"
...
def test_model_compatibility():
    ...
    try:
        ...
        ), "Analysis depth should be comprehensive"
        assert (
            request.include_health_assessment == True
        ), "Health assessment should be enabled"
        ...
        assert (
            response.results.vegetation_coverage == 50.0
        ), "Vegetation coverage should be 50%"
...
def test_analyzer_categories():
    ...
    try:
        ...
        for category in expected_categories:
            ...
            assert (
                "keywords" in categories[category]
            ), f"Category {category} should have keywords"
            assert (
                "weight" in categories[category]
            ), f"Category {category} should have weight"
        ...
        for keyword in expected_veg_keywords:
            assert (
                keyword in vegetation_keywords
            ), f"Vegetation should include {keyword}"
