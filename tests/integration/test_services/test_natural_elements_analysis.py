...
class MockVisionClient:
    ...
    def __init__(self):
        self.mock_labels = [
            type(
                "Label", (), {"description": "Tree", "score": 0.95, "topicality": 0.9}
            )(),
            type(
                "Label", (), {"description": "Grass", "score": 0.87, "topicality": 0.85}
            )(),
            type(
                "Label", (), {"description": "Sky", "score": 0.92, "topicality": 0.88}
            )(),
            type(
                "Label", (), {"description": "Green", "score": 0.78, "topicality": 0.75}
            )(),
            type(
                "Label", (), {"description": "Plant", "score": 0.82, "topicality": 0.80}
            )(),
            type(
                "Label",
                (),
                {"description": "Nature", "score": 0.89, "topicality": 0.85},
            )(),
            type(
                "Label",
                (),
                {"description": "Outdoor", "score": 0.91, "topicality": 0.87},
            )(),
            type(
                "Label", (), {"description": "Leaf", "score": 0.76, "topicality": 0.72}
            )(),
        ]
...
async def test_natural_element_analyzer():
    ...
    try:
        ...
        ), "Result should be NaturalElementsResult"
        ...
        print(
            f"   Built environment coverage: {result.built_environment_coverage:.1f}%"
        )

        assert (
            result.vegetation_coverage >= 0.0
        ), "Vegetation coverage should be non-negative"
        assert (
            result.vegetation_coverage <= 100.0
        ), "Vegetation coverage should not exceed 100%"
        ...
        if result.vegetation_health_score is not None:
            ...
            assert (
                0.0 <= result.vegetation_health_score <= 100.0
        ...
        if result.vegetation_health_metrics:
            ...
            assert isinstance(
                metrics, VegetationHealthMetrics
            ), "Should be VegetationHealthMetrics"
        ...
        if result.color_diversity_score is not None:
            ...
            assert (
                0.0 <= result.color_diversity_score <= 100.0
            ), "Color diversity should be 0-100"
        ...
        for category in result.element_categories:
            ...
            ), "Should be NaturalElementCategory"
            print(
                f"   {category.category_name}: {category.coverage_percentage:.1f}% coverage"
            )
        ...
        assert result.analysis_depth in [
            "basic",
            "comprehensive",
        ...
        assert (
            error_result.overall_assessment == "error"
        ), "Should handle errors gracefully"
        assert (
            "Analysis failed" in error_result.recommendations[0]
