...
def test_endpoint_logic():
    ...
    analysis_result = label_analysis_service.analyze_by_labels(
        labels=filtered_labels, analysis_depth="comprehensive", include_confidence=True
    )
    ...
    try:
        ...
        assert (
            response.image_hash == image_hash
        ), f"Expected hash {image_hash}, got {response.image_hash}"
        assert (
            len(response.results.category_analysis) > 0
        ), f"Expected categories > 0, got {len(response.results.category_analysis)}"
        assert (
            response.processing_time_ms >= 0
        ), f"Expected processing_time >= 0, got {response.processing_time_ms}"
...
def test_error_handling():
    ...
    try:
        result = label_analysis_service.analyze_by_labels(
            labels=[], analysis_depth="basic", include_confidence=True
        )
    ...
    try:
        ...
        result = label_analysis_service.analyze_by_labels(
            labels=invalid_labels, analysis_depth="basic", include_confidence=True
        )
