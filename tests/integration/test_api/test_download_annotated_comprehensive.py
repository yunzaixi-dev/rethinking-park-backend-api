...
def test_download_annotated_comprehensive():
    ...
    try:
        ...
        openapi_response = requests.get(
            f"{base_url.replace('/api/v1', '')}/openapi.json", timeout=5
        )
        ...
        response = requests.post(
            f"{base_url}/download-annotated", json=invalid_request, timeout=10
        )
        ...
        invalid_request_missing_hash = {"include_face_markers": True}
        ...
        if response.status_code == 422:
            ...
            print(
                f"   ❌ Should return 422 for missing required fields, got {response.status_code}"
            )
            return False
        ...
        response = requests.post(
            f"{base_url}/download-annotated", json=invalid_format_request, timeout=10
        )
        ...
        if response.status_code == 422:
            ...
            print(
                f"   ❌ Should return 422 for invalid format, got {response.status_code}"
            )
            return False

        # Test 5: Valid request structure with default values
        print("\n5. 📝 Valid Request Structure")
        valid_request = {"image_hash": "test_hash_for_structure_validation"}

        response = requests.post(
            f"{base_url}/download-annotated", json=valid_request, timeout=10
        )
        ...
        response = requests.post(
            f"{base_url}/download-annotated", json=custom_style_request, timeout=10
        )
        ...
        for fmt in formats:
            ...
            response = requests.post(
                f"{base_url}/download-annotated", json=format_request, timeout=10
            )
        ...
        for test in confidence_tests:
            ...
            response = requests.post(
                f"{base_url}/download-annotated", json=test_request, timeout=10
            )
            ...
            if test["should_pass"]:
                if response.status_code in [404, 200]:
                    ...
                    print(
                        f"   ❌ Valid confidence {test['confidence_threshold']} rejected"
                    )
                    return False
            else:
                if response.status_code == 422:
                    print(
                        f"   ✅ Invalid confidence {test['confidence_threshold']} rejected"
                    )
                else:
                    print(
                        f"   ❌ Invalid confidence {test['confidence_threshold']} should be rejected"
                    )
                    return False

        # Test 9: Response structure validation
        print("\n9. 📊 Response Structure Validation")
        structure_request = {"image_hash": "test_response_structure"}

        response = requests.post(
            f"{base_url}/download-annotated", json=structure_request, timeout=10
        )
