        ...
        try:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    print(
                        f"   Processing time: {result.get('processing_time_ms', 0)}ms"
                    )
                    ...
                    if "results" in result:
                        ...
                        print(
                            f"   Vegetation coverage: {results.get('vegetation_coverage', 0):.1f}%"
                        )
                        print(f"   Sky coverage: {results.get('sky_coverage', 0):.1f}%")
                        print(
                            f"   Water coverage: {results.get('water_coverage', 0):.1f}%"
                        )
                        print(
                            f"   Built environment: {results.get('built_environment_coverage', 0):.1f}%"
                        )

                        if results.get("vegetation_health_score"):
                            print(
                                f"   Vegetation health: {results['vegetation_health_score']:.1f}/100"
                            )

                        print(
                            f"   Analysis depth: {results.get('analysis_depth', 'unknown')}"
                        )
                        print(
                            f"   Total labels analyzed: {results.get('total_labels_analyzed', 0)}"
                        )
                        print(
                            f"   Overall assessment: {results.get('overall_assessment', 'unknown')}"
                        )

                        if results.get("recommendations"):
                            print(
                                f"   Recommendations: {len(results['recommendations'])} items"
                            )

                elif response.status == 404:
                    print(
                        "   ⚠️  Image not found - this is expected if using placeholder hash"
                    )
                    ...
                    error_detail = await response.json()
                    print(
                        f"   Error: {error_detail.get('detail', 'Service unavailable')}"
                    )
                ...
                if response.status == 200:
                    ...
                    if "results" in result:
                        ...
                        if results.get("vegetation_health_metrics"):
                            ...
                            print(
                                f"     Color health: {metrics.get('color_health_score', 0):.1f}"
                            )
                            print(
                                f"     Coverage health: {metrics.get('coverage_health_score', 0):.1f}"
                            )
                            print(
                                f"     Health status: {metrics.get('health_status', 'unknown')}"
                            )
                        ...
                        if results.get("seasonal_analysis"):
                            ...
                            print(
                                f"     Detected seasons: {seasonal.get('detected_seasons', [])}"
                            )
                            print(
                                f"     Primary season: {seasonal.get('primary_season', 'unknown')}"
                            )
                        ...
                        if results.get("dominant_colors"):
                            ...
                            print(
                                f"   Color diversity: {results.get('color_diversity_score', 0):.1f}"
                            )
                        ...
                        if results.get("element_categories"):
                            ...
                            for cat in categories[:3]:  # Show first 3
                                print(
                                    f"     {cat.get('category_name', 'Unknown')}: {cat.get('coverage_percentage', 0):.1f}%"
                                )

                elif response.status in [404, 503]:
                    print(
                        "   ⚠️  Expected error (image not found or service unavailable)"
                    )
                else:
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(
                f"{BASE_URL}{API_V1_STR}/images?limit=5"
            ) as response:
                if response.status == 200:
                    ...
                    if images:
                        ...
                        for img in images:
                            print(
                                f"     - {img.get('image_hash', 'unknown')[:16]}... ({img.get('filename', 'unknown')})"
                            )
