...
class TestEnhancedDetectionEndpoint:
    ...
    def test_enhanced_detection_success(
        ...
        from models.image import EnhancedDetectionResponse, EnhancedDetectionResult, BoundingBox, Point
        ...
        mock_get_image_info.assert_called_once_with(detection_request_data["image_hash"])
        mock_download_image.assert_called_once_with(detection_request_data["image_hash"])
    ...
    def test_enhanced_detection_image_not_found(self, client, detection_request_data):
        ...
        with patch('services.storage_service.get_image_info_by_hash') as mock_get_image_info:
...
class TestSimpleExtractionEndpoint:
    ...
    def test_simple_extraction_success(
        ...
        # Make request
        response = client.post(
            "/api/v1/extract-object-simple",
            json=extraction_request_data
        )
        ...
        assert data["result"]["extracted_image_url"] == "https://storage.googleapis.com/test/extracted.png"
    ...
    def test_simple_extraction_invalid_bounding_box(self, client):
        ...
        with patch('services.storage_service.get_image_info_by_hash') as mock_get_image_info:
            with patch('services.gcs_service.download_image') as mock_download_image:
                with patch('services.image_processing_service.validate_extraction_request') as mock_validate:
                    mock_get_image_info.return_value = Mock(image_hash="test_hash_12345678")
...
class TestLabelAnalysisEndpoint:
    ...
    def test_label_analysis_success(
        ...
        response = client.post(
            ...
            json=label_analysis_request_data
        )
    ...
    def test_label_analysis_vision_api_failure(self, client, label_analysis_request_data):
        """Test label analysis when Vision API fails"""
        with patch('services.storage_service.get_image_info_by_hash') as mock_get_image_info:
            with patch('services.gcs_service.download_image') as mock_download_image:
                with patch('services.vision_service.analyze_image') as mock_vision_analyze:
                    mock_get_image_info.return_value = Mock(image_hash="test_hash_12345678")
                    mock_download_image.return_value = b"fake_image_content"
                    ...
                    assert "Failed to get labels from Vision API" in response.json()["detail"]
...
class TestNaturalElementsEndpoint:
    ...
    def test_nature_analysis_success(
        ...
        response = client.post(
            ...
            json=nature_analysis_request_data
        )
    ...
    def test_nature_analysis_vision_disabled(self, client, nature_analysis_request_data):
        ...
        with patch('services.storage_service.get_image_info_by_hash') as mock_get_image_info:
            with patch('services.gcs_service.download_image') as mock_download_image:
                with patch('services.vision_service.is_enabled') as mock_vision_enabled:
                    ...
                    assert "Vision service is not available" in response.json()["detail"]
...
class TestAnnotatedImageEndpoint:
    ...
    def test_annotated_image_success(
        ...
        # Make request
        response = client.post(
            "/api/v1/download-annotated",
            json=annotation_request_data
        )
        ...
        assert data["result"]["annotated_image_url"] == "https://storage.googleapis.com/test/annotated.png"
    ...
    def test_annotated_image_validation_failure(self, client, annotation_request_data):
        ...
        with patch('services.storage_service.get_image_info_by_hash') as mock_get_image_info:
            with patch('services.gcs_service.download_image') as mock_download_image:
                with patch('services.enhanced_vision_service.detect_objects_enhanced') as mock_detection:
                    with patch('services.image_annotation_service.validate_annotation_request') as mock_validate:
                        mock_get_image_info.return_value = Mock(image_hash="test_hash_12345678")
                        mock_download_image.return_value = b"fake_image_content"
...
class TestBatchProcessingEndpoints:
    ...
    def test_get_batch_status_not_found(self, client):
        ...
        with patch('services.batch_processing_service.get_batch_status') as mock_get_status:
    ...
    def test_cancel_batch_job_not_found(self, client):
        ...
        with patch('services.batch_processing_service.cancel_batch_job') as mock_cancel_batch:
        ...
        with patch('services.storage_service.get_image_info_by_hash') as mock_get_image_info:
            with patch('services.gcs_service.download_image') as mock_download_image:
                with patch('services.enhanced_vision_service.detect_objects_enhanced') as mock_detection:
                    ...
                    response = client.post("/api/v1/detect-objects-enhanced", json=detection_request)
