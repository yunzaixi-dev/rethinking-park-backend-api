    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(15):  # 超过每分钟10次的上传限制
            try:
                ...
                data.add_field('file', test_data, filename=f'test{i}.jpg', content_type='image/jpeg')
...
async def test_duplicate_detection():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        data1.add_field('file', test_image_data, filename='original.jpg', content_type='image/jpeg')
        
        async with session.post(f"{API_BASE_URL}/api/v1/upload", data=data1) as response:
            if response.status == 200:
        ...
        data2.add_field('file', test_image_data, filename='duplicate.jpg', content_type='image/jpeg')
        
        async with session.post(f"{API_BASE_URL}/api/v1/upload", data=data2) as response:
            if response.status == 200:
...
async def test_hash_based_analysis():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        data = aiohttp.FormData()
        data.add_field('file', test_image_data, filename='analysis_test.jpg', content_type='image/jpeg')
        ...
        start_time = time.time()
        async with session.post(f"{API_BASE_URL}/api/v1/analyze", 
                              json=analysis_data,
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"{API_BASE_URL}/api/v1/stats") as response:
            if response.status == 200:
                ...
                print(f"     重复检测: {system_stats.get('duplicate_detection', False)}")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"{API_BASE_URL}/health") as response:
            if response.status == 200:
                ...
                print(f"     存储系统: 正常 ({storage_stats.get('total_images', 0)} 图像)")
