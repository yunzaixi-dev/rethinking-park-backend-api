...
def test_label_analysis_service():
    ...
    print(f"🌿 Vegetation coverage: {coverage_stats.get('vegetation_coverage', 0):.1f}%")
    print(f"☁️ Sky coverage: {coverage_stats.get('sky_coverage', 0):.1f}%")
    print(f"💧 Water coverage: {coverage_stats.get('water_coverage', 0):.1f}%")
    print(f"🏢 Built environment coverage: {coverage_stats.get('built_environment_coverage', 0):.1f}%")
    
    # Check insights
    insights = result.get('natural_element_insights', {})
    print(f"🎯 Dominant element: {insights.get('dominant_natural_element', 'Unknown')}")
    print(f"🌱 Environmental health score: {insights.get('environmental_health_score', 0):.1f}")
...
def test_label_analysis_request_model():
    ...
    try:
        ...
        print(f"✅ Default request created with categories: {request_default.target_categories}")
...
def test_category_mapping():
    ...
    for category, expected_labels in expected_categories.items():
        ...
        for expected_label in expected_labels:
            if not any(expected_label in actual_label for actual_label in actual_labels):
                print(f"❌ Expected label '{expected_label}' not found in category '{category}'")
                success = False
