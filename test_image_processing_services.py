...
class EnhancedDetectionResult:
    def __init__(self, object_id, class_name, confidence, bounding_box, center_point, area_percentage):
...
class FaceDetectionResult:
    def __init__(self, face_id, bounding_box, center_point, confidence, landmarks=None, emotions=None, anonymized=True):
...
class TestEnhancedVisionService:
    ...
    @pytest.mark.asyncio
    async def test_detect_objects_enhanced_success(self, service, mock_image_content, mock_vision_client):
    ...
    @pytest.mark.asyncio
    async def test_detect_objects_enhanced_disabled_service(self, service, mock_image_content):
    ...
    @pytest.mark.asyncio
    async def test_detect_objects_enhanced_vision_api_error(self, service, mock_image_content):
    ...
    @pytest.mark.asyncio
    async def test_confidence_filtering(self, service, mock_image_content, mock_vision_client):
        ...
        mock_high_conf.bounding_poly.normalized_vertices = [
            Mock(x=0.1, y=0.1), Mock(x=0.4, y=0.1), 
            Mock(x=0.4, y=0.6), Mock(x=0.1, y=0.6)
        ]
        ...
        mock_low_conf.bounding_poly.normalized_vertices = [
            Mock(x=0.5, y=0.5), Mock(x=0.8, y=0.5), 
            Mock(x=0.8, y=0.8), Mock(x=0.5, y=0.8)
        ]
    ...
    @pytest.mark.asyncio
    async def test_bounding_box_calculation_accuracy(self, service, mock_image_content, mock_vision_client):
    ...
    async def test_natural_elements_analysis(self, service, mock_image_content):
        ...
        mock_response = Mock()
        mock_response.label_annotations = [mock_tree_label, mock_grass_label, mock_sky_label]
    ...
    def test_color_analysis_accuracy(self, service):
        ...
        assert color_analysis["dominant_colors"]["green"] > color_analysis["dominant_colors"]["red"]
        assert color_analysis["dominant_colors"]["green"] > color_analysis["dominant_colors"]["blue"]
    ...
    def test_vegetation_health_calculation(self, service):
        ...
        health_score = service._calculate_vegetation_health(natural_elements, color_analysis)
...
class TestFaceDetectionService:
    ...
    @pytest.mark.asyncio
    async def test_detect_faces_enhanced_success(self, service, mock_image_content, mock_face_client):
    ...
    @pytest.mark.asyncio
    async def test_face_detection_confidence_filtering(self, service, mock_image_content):
        ...
        mock_high_conf_face.bounding_poly.vertices = [
            Mock(x=10, y=10), Mock(x=50, y=10), Mock(x=50, y=50), Mock(x=10, y=50)
        ]
        ...
        mock_low_conf_face.bounding_poly.vertices = [
            Mock(x=100, y=100), Mock(x=140, y=100), Mock(x=140, y=140), Mock(x=100, y=140)
        ]
    ...
    @pytest.mark.asyncio
    async def test_face_detection_anonymization(self, service, mock_image_content, mock_face_client):
...
class TestImageAnnotationService:
    ...
    def test_render_annotated_image_success(self, service, mock_image_content, mock_objects, mock_faces):
    ...
    def test_render_bounding_boxes_only(self, service, mock_image_content, mock_objects):
    ...
    def test_validate_annotation_request_success(self, service, mock_image_content, mock_objects, mock_faces):
    ...
    def test_validate_annotation_request_invalid_coordinates(self, service, mock_image_content):
        ...
        invalid_objects = [
            EnhancedDetectionResult(
                ...
                class_name="Person",
                confidence=0.9,
                bounding_box=BoundingBox(x=1.5, y=1.5, width=0.3, height=0.5),  # Invalid coordinates
    ...
    def test_custom_styling(self, service, mock_image_content, mock_objects, mock_faces):
